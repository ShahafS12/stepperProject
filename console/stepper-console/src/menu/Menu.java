...
public class Menu
{
    ...
    private void showPastRun(Scanner scanner)
    {
        ...
        if(!menuVariables.getStats().containsKey(actionFlow))
        {
            System.out.println("Invalid action");
            return;
        }
    ...
    private void showStatistics(Scanner scanner){
        ...
        if(actionFlow == 0)
            return;
        if(!menuVariables.getFlowExecutionsStatisticsMap().containsKey(menuVariables.getFlowNames().get(actionFlow - 1)))
        {
